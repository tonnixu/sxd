...
//合同审核
function auditBorrList(borrId, reason, url) {
    if (reason == '') {
        showMessage("必须填写理由！");
        return;
    }
...
//风控转件
function riskTransfer(borrIds, transfer) {
...
//导出函数
var borrowStautsAndprodcut = function(productId, borrStatus){
   if(productId == 1 && borrStatus == "BS011"){
       return "发放中";
   }else if(productId == 1 && borrStatus == "BS012"){
       return "发放失败";
   }else if(productId == 2 && borrStatus == "BS011"){
       return "放款中";
   }else if(productId == 2 && borrStatus == "BS012"){
       return "放款失败";
   }
};
